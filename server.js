...
const DATA_IMAGE_RE = /^data:image\/(\w+);base64,(.+)$/s;

async function writeDataUrlToFile(dataUrl, destPath) {
  const m = dataUrl.match(DATA_IMAGE_RE);
  if (!m) throw new Error('Not a recognizable image data URL');
  await writeFile(destPath, Buffer.from(m[2], 'base64'));
}
...
async function buildSegment(panel, index, dir, w, h) {
  ...
  const isHttpGif = panel.gifUrl && /^https?:\/\//.test(panel.gifUrl);
  // A sticker (from the background-cutout feature) is a static image handed
  // over as a data: URL, not a fetched GIF — same visual slot, different
  // ffmpeg input method (looped still frame instead of a looped animation).
  const isStickerImage = panel.gifUrl && DATA_IMAGE_RE.test(panel.gifUrl);
  const hasVisual = isHttpGif || isStickerImage;
  const isCard = panel.captionMode === 'card' || !hasVisual;
  ...
  if (isStickerImage) {
    const srcPath = path.join(dir, `sticker_${index}.png`);
    await writeDataUrlToFile(panel.gifUrl, srcPath);
